...
class ModbusUsbNumber(CoordinatorEntity[ModbusUsbCoordinator], NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        # Float registers must retain fractions; integer registers use the
        # nearest raw unit after undoing the display scale.
        ...
        if count == 1:
            ...
            await self.hass.async_add_executor_job(
                self.coordinator.write_registers_32bit, address, raw_value, data_type, slave
            )

...
def _template_filename(filename: str, *, add_extension: bool = False) -> str:
    """Accept only a plain YAML filename, never a path or another file type."""
    if (
...
def save_template_sync(hass: HomeAssistant, filename: str, content: str) -> dict[str, Any]:
    ...
    # Write in the same directory so os.replace is atomic. A failed write must
    # not truncate the previous template, and a symlink must not be followed.
    temporary_path = None
    try:

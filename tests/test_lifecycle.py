...
def unload_hass(*, success=True, preserve=False, other=False):
    ...
    hass = SimpleNamespace(
        ...
        config_entries=SimpleNamespace(async_unload_platforms=AsyncMock(return_value=success)),
...
async def test_failed_unload_keeps_services_and_coordinator():
    ...
    with patch.object(modbus_usb, "async_unregister_services", new_callable=AsyncMock) as unregister:
        assert not await modbus_usb.async_unload_entry(hass, SimpleNamespace(entry_id="hub"))
...
async def test_services_follow_loaded_hubs(other):
    ...
    with patch.object(modbus_usb, "async_unregister_services", new_callable=AsyncMock) as unregister:
        assert await modbus_usb.async_unload_entry(hass, SimpleNamespace(entry_id="hub"))
...
async def test_panel_registration_is_per_instance_and_retries_frontend_failure():
    ...
    with patch("homeassistant.components.frontend.async_register_built_in_panel", side_effect=[RuntimeError("not ready"), None, None]) as register:
